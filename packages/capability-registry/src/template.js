export function createCapabilityTemplate({ capabilityId = "example.capability", category = "example", owner = "Your Team" } = {}) {
  return {
    ...
    postconditionSchema: { type: "object", required: ["status", "evidence"], properties: { status: { type: "string", enum: ["VERIFIED", "FAILED", "UNCERTAIN"] }, evidence: {} } },
    ...
    confirmationPolicy: { mode: "NEVER", cannotBypass: false },
    ...
    networkConstraints: { domainPolicy: "NONE", allowedDomains: [], allowedMethods: [] },
    identities: { applications: [], packages: [] },
    trustedExecutionModality: "INTERNAL",
    availability: { available: true, reason: null, lastCheckedAt: null, check: () => true },
    preconditions: () => true,
    execute: async () => ({}),
